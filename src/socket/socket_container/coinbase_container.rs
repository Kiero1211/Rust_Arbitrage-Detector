    ...
    fn init_socket_connection(&mut self, symbol: &str) {
        ...
        match connect(Url::parse(&endpoint).unwrap()) {
            Ok((socket, _response)) => {
                ...
                let success_message = format!("[CoinBaseContainer] successfully connected to socket for symbol ({})", symbol);
                log_info!("{}", success_message);
            }
            Err(err) => {
                let error_log = format!("Cannot connect to CoinBase Websocket for symbol ({}), details: {}", symbol, err);
                log_error!("{}", error_log); 
            }
        };
